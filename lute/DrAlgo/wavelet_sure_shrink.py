...
def _universal_threshold(sigma: float, n: int) -> float:
    if sigma <= 0 or n <= 1:
        return 0.0
    return float(sigma * np.sqrt(2.0 * np.log(float(n))))


def _sureshrink_threshold(detail: np.ndarray, sigma: float) -> float:
    x = np.asarray(detail, dtype=np.float64).ravel()
    n = int(x.size)
    if n == 0 or sigma <= 0:
        return 0.0
    ...
    t2 = y
    sum_min = csum + t2 * (n - (idx + 1.0))
    risk = n * s2 + sum_min - 2.0 * s2 * (idx + 1.0)

    j = int(np.argmin(risk))
    t_sure = float(ax[j])
    t_univ = _universal_threshold(sigma, n)
...
class WaveletSureShrink2D(XhatDrAlgo):
    def __init__(
        self,
    ...
    def _fit_core(self, Xc: npt.NDArray, **kwargs: Any) -> None:
        ...
        sigma = float(self.thresh_mult) * float(sigma)
        ...
        for (cH, cV, cD) in details:
            ...
            tH = _sureshrink_threshold(cH, sigma)
            tV = _sureshrink_threshold(cV, sigma)
            tD = _sureshrink_threshold(cD, sigma)
        ...
        if getattr(self, "verbose", False):
            ...
            print(f"[WaveletSureShrink2D] level={level} sigma≈{sigma:.6g} detail nnz kept ≈ {kept:.2f}%")
            print(f"[WaveletSureShrink2D] rel_fro_error={err:.6g}")