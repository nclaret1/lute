...
def truncate_rank(A: np.ndarray, r: int) -> np.ndarray:
    U, s, Vt = np.linalg.svd(A, full_matrices=False)
    r = int(max(0, min(r, s.size)))
    if r == 0:
        return np.zeros_like(A)
    return (U[:, :r] * s[:r]) @ Vt[:r, :]


class RPCATVSVDADMM(LSDrAlgo):
    def __init__(
        self,
        n_components: int = 35,
        *,
    ...
    def _fit_core(self, Xc: npt.NDArray, **kwargs) -> None:
        ...
        if self.n_components is not None:
            L = truncate_rank(L, int(self.n_components))
            r = int(self.n_components) if self.n_components is not None else min(L.shape)
        if r > 0:
            U, s, Vt = np.linalg.svd(L, full_matrices=False)
            r = min(r, s.size)
            self.U_ = U[:, :r].astype(np.float32, copy=False)
            self.V_ = Vt[:r, :].T.astype(np.float32, copy=False)
            self.Sigma_ = np.diag(s[:r].astype(np.float32, copy=False))
        else:
            self.U_ = None
            self.V_ = None
            self.Sigma_ = None
