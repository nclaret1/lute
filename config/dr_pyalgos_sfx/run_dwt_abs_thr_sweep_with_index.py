#!/usr/bin/env python3
"""Orchestrate DWT absolute-threshold sweep WITH CrystFEL indexing.
...
Run with the psconda + lute-install environment active:
  ...
  python3 run_dwt_abs_thr_sweep_with_index.py
"""
...
ROWS     = 352
COLS     = 384
MAX_CXI_PEAKS = 2048   # max peaks per event in synthetic CXI

# ── YAML template ──────────────────────────────────────────────────────────────
YAML_TEMPLATE = """\
%YAML 1.3
---
title: "SFX: DWT abs_thr peakfind + baseline-v3r3 indexing"
...
def create_v3r3_input_cxi(work_dir: str, h5_path: str, rank0_cxi: str) -> Optional[str]:
    ...
    try:
        with h5py.File(rank0_cxi, "r") as f:
            dset = f["entry_1/data_1/data"]   # (N_cxi, 5632, 384)
            n_cxi = dset.shape[0]
            images = []
            ...
            for row in range(min(n_hits, n_cxi)):
                images.append(dset[row])   # (5632, 384)
    except OSError as e:
        print(f"  WARNING: Cannot read rank-0 CXI ({e}) — skipping synthetic CXI.",
              flush=True)
        return None

    if len(images) < n_hits:
        n_hits = len(images)
        baseline_peaks = baseline_peaks[:n_hits]

    images_arr = np.stack(images, axis=0).astype(np.float32)  # (N_hits, 5632, 384)

    # ── build peak arrays ────────────────────────────────────────────────────
    npeaks_arr = np.zeros(n_hits, dtype=np.int32)
    peak_x_arr = np.zeros((n_hits, MAX_CXI_PEAKS), dtype=np.float32)  # col (fs)
    peak_y_arr = np.zeros((n_hits, MAX_CXI_PEAKS), dtype=np.float32)  # global ss
    peak_i_arr = np.zeros((n_hits, MAX_CXI_PEAKS), dtype=np.float32)  # intensity
...
def main():
    for thr in DWT_PEAK_ABS_THR:
        ...
        yaml_path   = f"{YAML_DIR}/dionisio_dwt_peakfind_thr{tag}_indexed.yaml"
        ...
        html_out    = f"{work_dir}/dwt_abs_thr{tag}_peakfinder_indexed_r0016.html"
        ...
        # ── 4. build synthetic CXI (DWT images + v3r3-baseline peaks) ────────
