...
class RPCAAltProjThresh(LSDrAlgo):
    def __init__(
            self, 
    ...
    def __fit_core(
        ...
        nonzero_vals = S[np.abs(S) > 0]

        if nonzero_vals.size > 0:
            mean_S = nonzero_vals.mean()
            std_S = nonzero_vals.std()

            lower = mean_S - 3 * std_S
            upper = mean_S + 3 * std_S
            mask = (S >= lower) & (S <= upper)
            S = S * mask
            nonzero_vals = S[np.abs(S) > 0]
            perc_nonzero = 100 * nonzero_vals.size / S.size
            print("Sparsity: {:.4f} %".format(perc_nonzero))
